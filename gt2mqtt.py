...
#                log("Serial_gtp: %s"  %serial_gtp.hex())              
    ...
    def accOff_func(self):
      ...
      url = "http://api.map.baidu.com/routematrix/v2/driving?output=json&origins=" + latitude_str + "," + longitude_str + "&destinations=" + self.pos_home['latitude'] + "," + self.pos_home['longitude'] + "&coord_type=gcj02&tactics=12&ak=self.baidu_key"
      res = requests.get(url)
      log("response = %s" %res)
