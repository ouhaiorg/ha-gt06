...
class pipethread(threading.Thread):
    ...
    def __init__(self,source,mqtt):
        ...
        self.pos_home = { 'latitude': 27.97416, 'longitude': 120.70862 }
        self.radius_home = 100/111000
        self.deviceID = "noname"
        self.home = 'home'
        self.amap_key = "************"  #from lbs.amap.com get a web servicekey
...
#                log("Serial_gtp: %s"  %serial_gtp.hex())              
    ...
    def accOff_func(self):
      ...
      url = "http://api.map.baidu.com/routematrix/v2/driving?output=json&origins=" + latitude_str + "," + longitude_str + "&destinations=27.974037,120.708832" + "&coord_type=gcj02&tactics=12&ak=Kc37bOBVqEelGK7R8znT6HqY11B69kbt"
      res = requests.get(url)
      log("response = %s" %res)
...
class s2mqtt(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            mymqtt = mqtt2(mhost, mport, self.newsock, self.muser, self.mpassword)
            mymqtt.start()
